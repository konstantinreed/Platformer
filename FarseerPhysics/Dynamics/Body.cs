...
namespace FarseerPhysics.Dynamics
{
    ...
    public class Body : IDisposable
    {
		...
		public class Cache
		{
			...
			public void Reset()
			{
				BodyType = default(BodyType);
				Position = default(Vector2);
				Rotation = default(float);
				LinearVelocity = default(Vector2);
				AngularVelocity = default(float);
				GravityScale = default(float);
				UserData = default(object);
				Enabled = default(bool);
				FixedRotation = default(bool);
				SleepingAllowed = default(bool);
				LinearDamping = default(float);
				AngularDamping = default(float);
				Awake = default(bool);
				IsBullet = default(bool);
				IgnoreCCD = default(bool);
				IgnoreGravity = default(bool);
				Torque = default(float);

				Inertia = default(float);
				Mass = default(float);
				Force = default(Vector2);
				InvI = default(float);
				InvMass = default(float);
				SleepTime = default(float);
				Island = default(bool);
			}

			public void Clone(Cache cache)
			{
				BodyType = cache.BodyType;
				Position = cache.Position;
				Rotation = cache.Rotation;
				LinearVelocity = cache.LinearVelocity;
				AngularVelocity = cache.AngularVelocity;
				GravityScale = cache.GravityScale;
				UserData = cache.UserData;
				Enabled = cache.Enabled;
				FixedRotation = cache.FixedRotation;
				SleepingAllowed = cache.SleepingAllowed;
				LinearDamping = cache.LinearDamping;
				AngularDamping = cache.AngularDamping;
				Awake = cache.Awake;
				IsBullet = cache.IsBullet;
				IgnoreCCD = cache.IgnoreCCD;
				IgnoreGravity = cache.IgnoreGravity;
				Torque = cache.Torque;
		...
		{
			cache.BodyType = _bodyType;
			cache.Position = _xf.p;
			cache.Rotation = _sweep.A;
			cache.LinearVelocity = _linearVelocity;
			cache.AngularVelocity = _angularVelocity;
			cache.GravityScale = GravityScale;
			cache.UserData = UserData;
			cache.Enabled = _enabled;
			cache.FixedRotation = _fixedRotation;
			cache.SleepingAllowed = _sleepingAllowed;
			cache.LinearDamping = _linearDamping;
			cache.AngularDamping = _angularDamping;
			cache.Awake = _awake;
			cache.IsBullet = IsBullet;
			cache.IgnoreCCD = IgnoreCCD;
			cache.IgnoreGravity = IgnoreGravity;
			cache.Torque = _torque;

			cache.Inertia = _inertia;
			cache.Mass = _mass;
			cache.Force = _force;
			cache.InvI = _invI;
			cache.InvMass = _invMass;
			cache.SleepTime = _sleepTime;
			cache.Island = _island;
		}

		public void FromCache(Cache cache)
		{
			_bodyType = cache.BodyType;
			_linearVelocity = cache.LinearVelocity;
			_angularVelocity = cache.AngularVelocity;
			GravityScale = cache.GravityScale;
			UserData = cache.UserData;
			_enabled = cache.Enabled;
			_fixedRotation = cache.FixedRotation;
			_sleepingAllowed = cache.SleepingAllowed;
			_linearDamping = cache.LinearDamping;
			_angularDamping = cache.AngularDamping;
			_awake = cache.Awake;
			IsBullet = cache.IsBullet;
			IgnoreCCD = cache.IgnoreCCD;
			IgnoreGravity = cache.IgnoreGravity;
			_torque = cache.Torque;

			_inertia = cache.Inertia;
			_mass = cache.Mass;
			_force = cache.Force;
			_invI = cache.InvI;
			_invMass = cache.InvMass;
			_sleepTime = cache.SleepTime;
			_island = cache.Island;

			SetTransform(ref cache.Position, cache.Rotation);
		}
	}
}